...
void Servo_Move(double angle, double max_angle, int32_t pwm_channel)
{
    ...
    double dc = map_double(percent, 0, 100, 2.76, 12.75); // dc 2.5% -- 12.5%
    int32_t value = (int32_t)map_double(dc, 2.76, 12.75, 0, pow(2, PWM_RES_BITS_Servo)-1); //  
    // Serial.printf("Percent: %.3f    DC: %.3f   value: %d\n", percent, dc, value);
    // Serial.println(pow(2, PWM_RES_BITS_Servo)-1.0);
